...
class OldBooking(models.Model):
    """DEPRECATED: Old Booking model - use new Reservation + Booking instead."""
    member = models.ForeignKey("accounts.MemberProfile", on_delete=models.CASCADE, related_name="old_bookings")
    ...
    def __str__(self) -> str:
        return f"Old Booking #{self.id} - {self.member}"


class OldReservation(models.Model):
    """DEPRECATED: Old Reservation model - use new Reservation instead."""
    booking = models.ForeignKey(OldBooking, on_delete=models.CASCADE, related_name="old_reservations")
    facility = models.ForeignKey("management_portal.Facility", on_delete=models.PROTECT)
    ...
    assigned_officer = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="old_assigned_reservations",
    )
    ...
    def __str__(self) -> str:
        return f"Old Reservation #{self.id} for {self.facility}"


class ReservationEquipment(models.Model):
    """DEPRECATED: Use Reservation_Equipments instead."""
    reservation = models.ForeignKey(OldReservation, on_delete=models.CASCADE, related_name="old_equipments")
...
class OldTrainingSession(models.Model):
    """DEPRECATED: Use new TrainingSession instead."""
    title = models.CharField(max_length=100)
    coach = models.ForeignKey("accounts.CoachProfile", on_delete=models.CASCADE, related_name="old_sessions")
    facility = models.ForeignKey("management_portal.Facility", on_delete=models.PROTECT)
    start_time = models.DateTimeField()
    end_time = models.DateTimeField()
...
class SessionEnrollment(models.Model):
    """DEPRECATED: Use Session_Enrollment instead."""
    session = models.ForeignKey(OldTrainingSession, on_delete=models.CASCADE, related_name="old_enrollments")
    member = models.ForeignKey("accounts.MemberProfile", on_delete=models.CASCADE, related_name="old_enrollments")
    status = models.CharField(
        max_length=20,
        choices=[
            ("confirmed", "Confirmed"),
