...
First, we verified the active permissions to ensure the role configuration is
effective. Figure~\ref{fig:task4:show-grants} displays the \texttt{SHOW GRANTS}
output, confirming the specific privileges assigned to the user accounts.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{figure/task4/root.png}
  \\
  \textbf{Root User}
  \\
  \includegraphics[width=0.8\textwidth]{figure/task4/coach.png}
  \\
  \textbf{Coach User}
  \\
  \includegraphics[width=0.8\textwidth]{figure/task4/member.png}
  \\
  \textbf{Member User}
  \caption{Permission verification: \texttt{SHOW GRANTS} output demonstrating active privileges for \texttt{root},\texttt{coach} and \texttt{member} roles.}
  \label{fig:task4:show-grants}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{figure/task4/sql_coach_select.png}
...
Figure~\ref{fig:task4:sql-member-select-view} confirms that, after revoking
base-table access (Figure~\ref{fig:task4:sql-member-select-booking-denied}), the
member can still retrieve necessary booking details through the view, without
exposing other members' data.
...
To further validate the effectiveness of the Row-Level Security, we conducted a negative test where Member 1 attempts to explicitly retrieve records belonging to another member (e.g., Member 2). As demonstrated in Figure~\ref{fig:task4:member-isolation}, the result is an empty set (or equivalent denial), confirming that the view successfully filters out unauthorized data even when specifically requested.

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{figure/task4/sc4_member_deny.png}
  \caption{Isolation verification: Member 1 attempts to access Member 2's booking data through the view, resulting in error (Access Denied).}
  \label{fig:task4:member-isolation}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\textwidth]{figure/task4/403.png}
