...
\begin{figure}[H]
    ...
    \caption{CREATE TABLE execution (Student, Staff, External\_Visitor)}
...
\begin{figure}[H]
    ...
    \caption{CREATE TABLE execution (Member\_Email, Member\_Phone, Coach\_Email, Coach\_Phone)}
...
\begin{figure}[H]
    ...
    \caption{CREATE TABLE execution (Facility, Coach, Maintenance)}
...
\begin{figure}[H]
    ...
    \caption{CREATE TABLE execution (Visitor\_Application, Training\_Session, Session\_Enrollment, Reservation\_Equipments)}
...
table with a minimum of 6–10 rows of data. The following screenshot verifies the
data population by showing the row counts for the key tables in the database.
...
Show all equipment and any reservations associated with them (ensuring all equipment types are listed).
